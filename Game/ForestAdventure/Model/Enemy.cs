...
namespace ForestAdventure.Model
{
    ...
    {
        private readonly float movementBorderMaximal;
        private readonly float movementBorderminimal;
        ...
        private void HandleMovement(float frameTime)
        {
            switch (leftright)
            {
                case "left":
                    ...
                    if (MinX < movementBorderminimal) leftright = "right";

                    break;
                case "right":
                    MinX += frameTime * 0.1f;
                    MinX = Math.Max(MinX, -1f);
                    if (MinX > movementBorderMaximal) leftright = "left";
